...
int app_sensor_analog_sample(void)
{
	...
	for (size_t i = 0; i < channels_count; i++) {
		...
		LOG_DBG("ch %d: cnt: %d, mean: %.1f, raw: %.1f",
...
int app_button_aggreg(void)
{
	int ret;

	for (size_t i = 0; i < APP_DATA_BUTTON_COUNT; i++) {

		if (g_app_data.button[i].event_count == 0) {
			ret = ctr_rtc_get_ts(&g_app_data.button[i].timestamp);
			if (ret) {
		...
		if (g_app_data.button[i].event_count < APP_DATA_MAX_BUTTON_EVENTS) {

			int event_count = g_app_data.button[i].event_count;

			struct app_data_button_event event =
				g_app_data.button[i].events[event_count];

			event.open = g_app_data.button[i].open_count;
			event.close = g_app_data.button[i].close_count;

			g_app_data.button[i].events[event_count] = event;

			LOG_DBG("Button %d, count: %d, open: %d, close: %d", i + 1, event_count,
				event.open, event.close);

			g_app_data.button[i].event_count++;
		} else {
			LOG_WRN("Event buffer full");
		}

		g_app_data.button[i].open_count = 0;
		g_app_data.button[i].close_count = 0;
	}

	for (size_t i = 0; i < APP_DATA_BUTTON_COUNT; i++) {
		g_app_data.button[i].sample_count = 0;
	}
...
int app_sensor_analog_aggreg(void)
{
	...
	for (size_t i = 0; i < APP_CONFIG_CHANNEL_COUNT; i++) {
		...
		if (g_app_data.channel[i].measurement_count < APP_DATA_ANALOG_MAX_MEASUREMENTS) {
			...
			if (g_app_config.channel_calib_a1 == 1 ||
			    g_app_config.channel_calib_a10 == 1 ||
			    g_app_config.channel_calib_v5 == 1 ||
			    g_app_config.channel_calib_v24 == 1) {

				aggreg_sample(
					g_app_data.channel[i].samples_rms,
					g_app_data.channel[i].sample_count,
					&g_app_data.channel[i].measurements_rms[measurement_count]);

				if (i > 1) {
					LOG_INF("Calibration: A1 preset: %d A10 preset: %d",
						g_app_config.channel_calib_a1,
						g_app_config.channel_calib_a10);

					// Auto calibration for 1 amp
					if (g_app_config.channel_calib_a1 == 1) {
						g_app_config.channel_calib_x0[i] =
							g_app_data.channel[i]
								.measurements_rms[measurement_count]
								.avg;

						LOG_INF("Calibration: A1 set, raw avg: %.1f",
							g_app_data.channel[i]
								.measurements_rms[measurement_count]
								.avg);
					}

					// Auto calibration for 10 amp
					if (g_app_config.channel_calib_a10 == 1) {
						g_app_config.channel_calib_x1[i] =
							g_app_data.channel[i]
								.measurements_rms[measurement_count]
								.avg;
				...
				if (i < 2) {
					LOG_INF("Calibration: V5 preset: %d V24 preset: %d",
						g_app_config.channel_calib_v5,
						g_app_config.channel_calib_v24);

					// Auto calibration for 5 volt
					if (g_app_config.channel_calib_v5 == 1) {
						g_app_config.channel_calib_x0[i] =
							g_app_data.channel[i]
								.measurements_rms[measurement_count]
								.avg;

						LOG_INF("Calibration: V5 set, raw avg: %.1f",
							g_app_data.channel[i]
								.measurements_rms[measurement_count]
								.avg);
					}

					// Auto calibration for 24 volt
					if (g_app_config.channel_calib_v24 == 1) {
						g_app_config.channel_calib_x1[i] =
							g_app_data.channel[i]
								.measurements_rms[measurement_count]
								.avg;

						LOG_INF("Calibration: V24 set, raw avg: %.1f",
							g_app_data.channel[i]
								.measurements_rms[measurement_count]
								.avg);
					}
	...
	if (g_app_config.channel_calib_a1 == 1) {
		g_app_config.channel_calib_a1 = 0;
		h_rev_commit();
	}
	if (g_app_config.channel_calib_a10 == 1) {
		g_app_config.channel_calib_a10 = 0;
		h_rev_commit();
	}

	if (g_app_config.channel_calib_v5 == 1) {
		g_app_config.channel_calib_v5 = 0;
		h_rev_commit();
	}
	if (g_app_config.channel_calib_v24 == 1) {
		g_app_config.channel_calib_v24 = 0;
		h_rev_commit();
	}
...
int app_sensor_analog_clear(void)
{
	...
	for (int i = 0; i < APP_DATA_BUTTON_COUNT; i++) {
		int event_count = g_app_data.button[i].event_count;
		struct app_data_button_event event = g_app_data.button[i].events[event_count];
		...
		g_app_data.button[i].events[event_count] = event;
		g_app_data.button[i].event_count = 0;
	}
	...
	for (int j = 0; j < MIN(APP_DATA_W1_THERM_COUNT, ctr_ds18b20_get_count()); j++) {
		if (g_app_data.w1_therm.sensor[j].sample_count < APP_DATA_W1_THERM_MAX_SAMPLES) {
			...
			if (ret) {
				...
				LOG_DBG("Temperature: %.1f C", temperature);
			}
